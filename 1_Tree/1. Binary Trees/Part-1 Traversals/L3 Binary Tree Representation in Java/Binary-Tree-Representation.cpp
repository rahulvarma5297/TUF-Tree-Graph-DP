...
class Solution{
public:
    node* build_Tree(int i, int n, vector<int>& vec){
        ...
        if(i < n){
            ...
            root->left = build_Tree(2*i+1, n, vec);
            root->right = build_Tree(2*i+2, n, vec);
        }
        return root;
    }
    void create_tree(node* root0, vector<int> &vec){
        ...
        int n = vec.size();
        root0->left = build_Tree(1, n, vec);
        root0->right = build_Tree(2, n, vec);
    }
